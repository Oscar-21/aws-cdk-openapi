...
public class ApiDocsS3Resources extends AbstractCustomLambdaRuntime<ApiStack> {
  ...
  public void buildImage(ApiStack stack) {
    /**
     * Generate the documentation from the OpenAPI Specification docker build -t openapidoc . docker
     * run -it openapidoc sh
     */
    ...
    BundlingOptions.Builder apiDocBuilderOptions = BundlingOptions.builder()
        .command(defaultApiPackagingInstructions).image(apDocImage).local(new ILocalBundling() {
          @Override
    ...
    ISource apiDocSource = Source.asset("../app/api-docs", AssetOptions.builder()
        .bundling(apiDocBuilderOptions.command(apiDocPackagingInstructions).build()).build());
    ...
    BucketDeployment bucketDeployment = BucketDeployment.Builder
        ...
        .cacheControl(Arrays.asList(CacheControl.setPublic(),
            CacheControl.maxAge(Duration.seconds(0)), CacheControl.sMaxAge(Duration.seconds(0))))
        .build();
